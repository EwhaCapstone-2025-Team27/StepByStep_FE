...
export const aiApi = {
  stream: async ({ query, topk = 8, friendStyle = true, onMessage, onDone, signal }) => {
    ...
    const res = await fetch(toURL('/api/chat/stream'), {
      method: 'POST',
      headers: {
  ...
  generateQuiz: async ({ keywords = [], perKeyword = 2, seed = Date.now() & 0xffff } = {}) => {
    const body = {
      ...(Array.isArray(keywords) && keywords.length ? { keywords: keywords.map((kw) => String(kw)) } : {}),
      ...(perKeyword != null ? { perKeyword } : {}),
      ...(seed != null ? { seed } : {}),
    };

    return fetchJSON('/api/ai/quiz', { method: 'POST', body });
  },
};
...
const consumeSseResponse = async (res, { onMessage }) => {
  const reader = res.body?.getReader?.();
  if (!reader) {
    const text = await res.text();
    processSseBuffer(text, onMessage);
    return;
  }
