...
// 기본값을 API와 동일하게 맞춰 AI 서버가 따로 설정되지 않아도 동작하도록 한다.
const RAG_API = process.env.EXPO_PUBLIC_RAG_API || API;
...
export const badgeApi = {
  ...
  purchase: (badgeId) =>
      fetchJSON(`/api/badges/${badgeId}/purchase`, { method: 'POST', body: { id: badgeId } }),
};
...
export const moderationApi = {
  checkText: (body) => fetchJSON('/api/moderation/check', { method: 'POST', body }),
  guardPost: (body) => fetchJSON('/api/moderation/guard-post', { method: 'POST', body }),
  guardComment: (body) => fetchJSON('/api/moderation/guard-comment', { method: 'POST', body }),
  guardInput: (body) => fetchJSON('/api/moderation/guard-input', { method: 'POST', body }),
  guardOutput: (body) => fetchJSON('/api/moderation/guard-output', { method: 'POST', body }),
  filterSnippets: (body) => fetchJSON('/api/moderation/filter-snippets', { method: 'POST', body }),
  guardBatch: (body) => fetchJSON('/api/moderation/guard-batch', { method: 'POST', body }),
};

/* -------------------- 퀴즈 (BE) -------------------- */
export const quizApi = {
  getKeywords: () => fetchJSON('/api/quiz/kewords'),
  createAttempt: ({ mode = 'RANDOM', scenarioId, questionCount = 2 } = {}) =>
      fetchJSON('/api/quiz/attempts', {
        method: 'POST',
        body: {
          mode,
          questionCount,
  ...
  submitResponse: ({ attemptId, questionId, optionId }) =>
      fetchJSON(`/api/quiz/attempts/${attemptId}/responses`, {
        method: 'POST',
        body: { questionId, optionId },
      }),
  getAttempt: (attemptId) => fetchJSON(`/api/quiz/attempts/${attemptId}`),
};